...
static int fdjac_simple(const gsl_vector *x, gsl_multifit_function_fdf *fdf,
...
static int
fdjac_simple(const gsl_vector *x, gsl_multifit_function_fdf *fdf,
      const gsl_vector *f, gsl_matrix *J)
{
  int status = 0;
  size_t i, j;
  ...
  for (j = 0; j < fdf->p; ++j)
    {
      ...
      status += gsl_multifit_eval_wf(fdf, x, NULL, &v.vector);
#endif
      if (status)
        return status;
...
int
gsl_multifit_fdfsolver_dif_df_simple(const gsl_vector *x, gsl_multifit_function_fdf *fdf,
                              const gsl_vector *f, gsl_matrix *J)
{
  return fdjac_simple(x, fdf, f, J);
} /* gsl_multifit_fdfsolver_dif_df() */
...
int
gsl_multifit_fdfsolver_dif_fdf_simple(const gsl_vector *x, gsl_multifit_function_fdf *fdf,
                               gsl_vector *f, gsl_matrix *J)
{
  ...
  status = fdjac_simple(x, fdf, f, J);
  if (status)
    return status;

  return status;
} /* gsl_multifit_fdfsolver_dif_fdf_simple() */
